...
dados = pd.read_csv('E:/Nova Biblioteca/NovaBiblioteca/GitHub/TrabPython/AVF_Treinamento_de_Classificador/breast-cancer(1)(1).csv', sep=',', encoding='iso-8859-1')
print(dados.info())

numerico_columns = ['deg-malig']
categorico_columns = ['age', 'menopause', 'tumor-size', 'inv-nodes', 'node-caps', 'breast', 'breast-quad', 'irradiat']

normalizadorStandardScaler = preprocessing.StandardScaler()
dados[numerico_columns] = normalizadorStandardScaler.fit_transform(dados[numerico_columns])

convertLabelEncoder = preprocessing.LabelEncoder()
for column in categorico_columns:
    dados[column] = convertLabelEncoder.fit_transform(dados[column])

convertOneHotEncoder = preprocessing.OneHotEncoder(sparse_output=False)
categorizar_dados = convertOneHotEncoder.fit_transform(dados[categorico_columns])

normalizar_dados = np.concatenate([dados[numerico_columns].values, categorizar_dados], axis=1)

columns = numerico_columns + list(convertOneHotEncoder.get_feature_names_out(categorico_columns))
normalizar_df = pd.DataFrame(data=normalizar_dados, columns=columns)

print(f'Dados Normalizados: {normalizar_df}')

balanced = RandomOverSampler(random_state=42)
resampled_a, resampled_b = balanced.fit_resample(normalizar_df, dados['Class'])
